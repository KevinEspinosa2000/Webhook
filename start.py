...
ENDPOINT = os.getenv("Webhook_Endpoint", "https://webhook.site/f527b9a8-210a-4bde-87d0-d5187a1923c8")
INTERVAL = int(os.getenv("Intervalo_Webhook", 5))  #Segundos

def send_message():
    while True:
        try:
            response = requests.post(ENDPOINT, json={"message": "Webhook usando Flask", "timelegible": time.strftime("%Y-%m-%d %H:%M:%S")})
            print(f"Mensaje enviado: {response.status_code}")
            estado_previo = response.status_code
            print(f"Status Previo: {estado_previo}")
